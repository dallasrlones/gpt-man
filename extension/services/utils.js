...
const formatDocIfExists = () => {
    ...
    if (Object.keys(state.outline).length > 0) {
        ...
        const lastP = selectorDocument().querySelectorAll('p').length - 1;

        if (lastP > -1) {
            const lastPSelector = selectorDocument().querySelectorAll('p')[lastP];
            lastPSelector.parentElement.parentElement.children.forEach(child => {
                child.classList.add('glowing-text');
            });
            lastPSelector.parentElement.parentElement.scrollIntoView();
        } else {
            const lastH3 = selectorDocument().querySelectorAll('h3').length - 1;
            const lastH3Selector = selectorDocument().querySelectorAll('h3')[lastH3];
            lastH3Selector.parentElement.parentElement.children.forEach(child => {
                child.classList.add('glowing-text');
            });
            lastH3Selector.parentElement.parentElement.scrollIntoView();
        }
    }
};
